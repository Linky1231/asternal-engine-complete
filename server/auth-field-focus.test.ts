...
describe("foco de los campos de acceso", () => {
  it("usa un único borde y evita anillos o contornos nativos duplicados", () => {
    expect(AUTH_FIELD_FOCUS_CLASS).toContain("border-transparent");
  ...
  it("dibuja el contorno activo sobre todo el campo con un radio uniforme", () => {
    expect(AUTH_FIELD_FOCUS_OUTLINE_CLASS).toContain("absolute");
    expect(AUTH_FIELD_FOCUS_OUTLINE_CLASS).toContain("inset-0");
    expect(AUTH_FIELD_FOCUS_OUTLINE_CLASS).toContain("rounded-[inherit]");
    expect(AUTH_FIELD_FOCUS_OUTLINE_CLASS).toContain("border-primary/45");
  });
